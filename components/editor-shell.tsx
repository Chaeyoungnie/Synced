...
export function EditorShell({ sampleMode = false, workspaceId = null }: { sampleMode?: boolean; workspaceId?: string | null }) {
  ...
  useEffect(() => {
    function handleKeyDown(e: KeyboardEvent) {
      ...
      if ((e.metaKey || e.ctrlKey) && e.key === 's') { e.preventDefault(); setSaveState('saving'); saveFile(activeFile, contents[activeFile] || ''); if (activeFileId) saveVersion(activeFileId, contents[activeFile] || ''); setTimeout(() => setSaveState('saved'), 800); toast('File saved', 'success') }
