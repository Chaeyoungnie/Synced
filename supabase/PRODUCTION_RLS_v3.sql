-- ============================================
-- PRODUCTION RLS v3 (SIMPLE - NO RECURSION)
-- ============================================
-- Key insight: collaborators table has RLS
-- DISABLED because it is just a mapping table.
...
DO $$ BEGIN DROP POLICY IF EXISTS "collab_select" ON profiles; EXCEPTION WHEN undefined_object THEN NULL; END $$;
...
DO $$ BEGIN DROP POLICY IF EXISTS "collab_select" ON workspaces; EXCEPTION WHEN undefined_object THEN NULL; END $$;
...
DO $$ BEGIN DROP POLICY IF EXISTS "collab_select" ON files; EXCEPTION WHEN undefined_object THEN NULL; END $$;
...
DO $$ BEGIN DROP POLICY IF EXISTS "collab_select" ON collaborators; EXCEPTION WHEN undefined_object THEN NULL; END $$;
...
DO $$ BEGIN DROP POLICY IF EXISTS "collab_select" ON file_versions; EXCEPTION WHEN undefined_object THEN NULL; END $$;
...
DO $$ BEGIN DROP POLICY IF EXISTS "collab_select" ON messages; EXCEPTION WHEN undefined_object THEN NULL; END $$;
...
DO $$ BEGIN DROP POLICY IF EXISTS "collab_select" ON activities; EXCEPTION WHEN undefined_object THEN NULL; END $$;
...
DO $$ BEGIN DROP POLICY IF EXISTS "collab_select" ON invitations; EXCEPTION WHEN undefined_object THEN NULL; END $$;
...
-- STEP 2: DISABLE RLS on collaborators (mapping table, not sensitive)
ALTER TABLE collaborators DISABLE ROW LEVEL SECURITY;

-- Enable RLS on everything else
...
-- Collaborators can read (collaborators table has RLS disabled, so subquery works)
...
-- STEP 5: FILES
CREATE POLICY files_all ON files FOR ALL USING (
...
CREATE POLICY fv_member ON file_versions FOR ALL USING (
  EXISTS (
    ...
    AND (w.owner_id = auth.uid()
      OR EXISTS (SELECT 1 FROM collaborators WHERE workspace_id = w.id AND user_id = auth.uid()))
  )
);
...
-- STEP 11: Populate missing emails
